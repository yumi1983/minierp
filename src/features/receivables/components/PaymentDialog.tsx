...
import type { CreditAccountWithDetail, PaymentMethod } from '../types'
...
export function PaymentDialog({ accountId, onClose }: Props) {
  const { getDetail, registerPayment, uploadVoucher } = useReceivables()
  const [detail, setDetail] = useState<CreditAccountWithDetail | null>(null)
      ...
      <DialogContent className="flex flex-col w-full max-w-lg max-h-[92vh] p-0 gap-0">
            ...
            <>
              ...
              <div className="rounded-lg bg-muted/50 p-3 space-y-1 text-sm">
                <div className="flex justify-between gap-2">
                  ...
                  <span className="font-medium text-right">{detail.customer_name}</span>
                </div>
                <div className="flex justify-between gap-2">
                  <span className="text-muted-foreground shrink-0">Nota de Pedido</span>
                  <span className="font-mono text-xs font-medium">{detail.sale_code}</span>
                </div>
                <div className="flex justify-between gap-2">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs">Voucher / imagen del cobro</Label>
                  <input
                    ref={fileRef}
                    type="file"
                    accept="image/*"
              ...
              {detail.payments.length > 0 && (
                <div className="space-y-1.5">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Historial de cobros</p>
        ...
        <DialogFooter className="px-6 py-4 border-t shrink-0 sm:justify-end gap-2">
          ...
          >
            ...
            {uploadingVoucher ? 'Subiendo voucher...' : 'Registrar cobro'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
